...
abbrev: AMBI
docname: draft-ietf-mboned-ambi-01
date: 2020-10-30
category: std
...
Each manifest MUST be delivered in a way that provides cryptographic integrity guarantees of the authenticity of the manifest.
 ...
 * the authenticity of the manifests
 * the authenticity of the digest profile used for construction of the packet digests
 * the difficulty of generating a collision for the packet digests contained in the manifest.

This document defines a YANG {{RFC7950}} module that augments the DORMS {{I-D.draft-ietf-mboned-dorms-00}} YANG module to provide a way to communicate a digest profile, described in {{ref-profile}}, for construction of the packet digests, described in {{ref-digests}}.
When obtaining the digest profile by using DORMS, the authenticity of the data stream relies on a trust relationship with the DORMS server, since that anchors the authenticity of the digest profile for constructing packet digests.
...
## Buffering and Validation Windows
...
Once a data packet is authenticated, the corresponding packet digest can be discarded and the data packet can be further processed by the receiving application or forwarded through the receiving network.
...
A different manifest might provide the same packet digest with the same packet sequence number, but the digest remains consumed if it has been used to authenticate a data packet.
...
The YANG model in {{ref-yang}} provides a mechanism for senders to communicate the sender's recommendation for buffering durations, when using DORMS.
...
However receivers MAY deviate from the values recommended by the sender for a variety of reasons.
Decreasing the buffering durations recommended by the server increases the risk of losing packets, but can be an appropriate tradeoff for specific network conditions and hardware constraints on some devices.
...
The purpose for this recommendation is to preserve the capability of receivers to use techniques for available bandwidth detection or network congestion based on observation of packet times.
Examples of such techniques include paced chirping and pathrate.
...
TBD: there should also be a way to specify that only packets to a specific UDP port are applicable.
...
Follow-up on this after the first reference implementation...
...
When the digest profile indicates that IP payloads are validated, the IP payload of the packet is used, using the outermost IP layer that contains the (S,G) corresponding to the (S,G) protected by the manifest.
...
Zeroes if using a protocol that does not use source ports.

#### Destination Port

The destination port of the packet.
Zeroes if using a protocol that does not use destination ports.
...
But I suspect there's a better approach than UDP-or-not, so it's this way for now, with hopes of finding something better in the next version.
...
The payload type is configurable because when sending UDP, some legacy networks may strip the UDP option space, and it's necessary to provide a manifest stream capable of authentication that can interoperate with these networks.
...
The Refresh Deadline field from the manifest is used to signal to receivers the need to transition.
...
After the refresh deadline has expired, a manifest stream MAY end.
...
TBD: add ALTA to the list when and if it gets further along {{I-D.draft-krose-mboned-alta-01}}.
...
An HTTPS stream carrying the 'application/ambi' media type is composed of a sequence of binary AMBI manifests.
It is RECOMMENDED to use Chunked encoding.
...
TBD: DTLS {{RFC6347}} can provide authentication for datagrams, so if manifests can be constructed to fit within datagrams, it is an appropriate choice.
(IPSec is similar--worth adding as an option?).

This option provides no native redundancy or retransmission, but packet digests can be repeated in different manifests to provide some resilience to loss.
...
TBD: should we therefore not support this case? (probably still worthwhile--the manifests can still contain redundant hashes)

## DTLS + FECFRAME

DTLS for manifests that do not fit into single-packet payloads can still be delivered by using FECFRAME {{RFC6363}}, particularly Reed-Solomon {{RFC6865}} or possibly Raptor {{RFC6681}}.
This has some advantages compared to HTTPS because of the absence of HOL-blocking, while providing for tunable redundancy.
This has some advantages relative to DTLS because of overhead reduction and non-integer redundancy tunability (e.g. 1.5 becomes a viable redundancy factor).
